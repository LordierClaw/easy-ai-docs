...
if($Context.action -notin @('install','repair')){exit 0}
...
foreach($tool in @('git.exe','node.exe','npm.cmd')){
  if(!(Get-Command $tool -CommandType Application -ErrorAction SilentlyContinue)){throw ('Thiếu '+$tool+'. Chạy lại bước contract runtime và kiểm tra bundle/PATH của EasyAI.')}
}
