...
2. Sau duyệt, gọi `execute_plan {}`. `configure-mcp.ps1` kiểm tra CLI và cấu hình TOML qua chính Codex, kiểm tra bản cấu hình dự kiến trong folder riêng, rồi thay file có backup để thêm duy nhất server `atlassian` với URL `https://mcp.atlassian.com/v2/mcp`. Nếu bản dự kiến không hợp lệ (ví dụ cấu hình dùng inline table), giữ nguyên file gốc để AI đánh giá cách sửa. Đã đúng thì dùng lại; trùng tên nhưng khác cấu hình thì dừng để đánh giá.
